...
WinNativeWindow::WinNativeWindow(const int x, const int y, const int width, const int height)
    : hWnd(nullptr)
{
    //Trick for transparency:
    //ALL Pixels in this (exact) color will be transparent
    COLORREF transparentColor = RGB(0,0,0);

	//The native window technically has a background color. You can set it here
    HBRUSH windowBackground = CreateSolidBrush(transparentColor);
    ...
    // Gets back the windows borders & shadow
