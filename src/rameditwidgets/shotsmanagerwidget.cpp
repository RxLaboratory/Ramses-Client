...
void ShotsManagerWidget::newShot(RamObject *shot)
{
    if (!shot) return;
    if (shot->uuid() != "")
    {
...
void ShotsManagerWidget::shotRemoved(RamObject *s)
{
    removeData(s->uuid());
}
...
void ShotsManagerWidget::addShots(RamSequence *sequence)
{
    ...
    {
        newShot(sequence->at(i));
    }
    _sequencesConnections << connect(sequence, &RamSequence::objectAdded, this, &ShotsManagerWidget::newShot);
    _sequencesConnections << connect(sequence, SIGNAL(objectRemoved(RamObject*)), this, SLOT(shotRemoved(RamObject*)));
